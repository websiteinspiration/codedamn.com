...
}).with('password', 'cpassword').unknown(false).required()

export const oauthSchema = Joi.object().keys({
	name: name.required(), 
	username: username.required(),
	email: email.required(),
...
}).unknown(false).without('facebookID', 'googleID').without('googleID', 'facebookID').required()